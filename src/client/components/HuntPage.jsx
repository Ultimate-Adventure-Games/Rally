...
const HuntPage = (props) => {
  ...
  const { hunts, events, setEvents, user } = useContext(AppContext);
  ...
  const onSelectEventHandler = (id) => {
    if (!map) return;
    const { event_pos, event_name, event_riddle } = events.find(
  ...
  const uploadPhotoHandler = async (file, id) => {
    ...
    const response = await fetch(
      `http://localhost:3000/api/photos/image-upload/` +
        id +
        "/" +
        user.user_id,
      {
    ...
    <div className="hunt-page">
      <div className="map-container">
        ...
        {isLoaded ? (
          ...
          >
            {events.map((event) => (
              <Marker key={event.event_id} position={event.event_pos} />
            ))}
            {selectedEvent ? selectedEvent : <></>}
            <Polyline path={eventPath} options={pathOptions} />
          </GoogleMap>
        ) : (
          <p>loading map...</p>
        )}
      ...
      <div className="list-item-section">
        {events.map((event) => (
          <EventListItem
            ...
            description={event.event_riddle}
            onSelect={onSelectEventHandler.bind(this, event.event_id)}
          />
        ))}
      </div>
      <Link
